...
按 `qwen3.7-flash` 价格口径估算：正常输入 `0.2 元/百万 token`，缓存命中输入 `0.02 元/百万 token`，输出 `0.8 元/百万 token`。

以每次 2000 token、输出 56 token、缓存命中比例约 30.5% 估算，单次 AI 分析约 `0.000327 元`；一次 3D 打印按 30 次分析计算约 `0.0098 元`，**不到 1 分钱**。即使直接按实际 1736 token 计算，一次也仅约 `0.00866 元`（约 0.87 分钱）。

**结论：AI 打印监控成本非常低，基本可以忽略。**
