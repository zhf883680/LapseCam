...
**Tech Stack:** Go 1.26、modernc SQLite、net/http（Go 1.22+ 方法路由）、原生 JS 单文件页面（web/index.html）。
...
func (s *Service) cleanupFrames(stats *CleanupStats) {
	...
	for rows.Next() {
		...
		if _, err := os.Stat(framesDir); err == nil {
			if sz, err := dirSize(framesDir); err == nil {
